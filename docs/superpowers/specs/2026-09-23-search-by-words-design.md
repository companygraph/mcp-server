...
Every word counts. There is no stop list: a list of words to ignore is a second vocabulary to keep, in one language, and a word that occurs in every entity costs a query nothing but breadth. A client that wants fewer words sends fewer words, and the chat's rule, in its own repository, will say to search with the words that carry the meaning. A query whose words all reduce to nothing, punctuation alone, is refused as `invalid_argument` on `query`, the refusal an empty query gets today.
