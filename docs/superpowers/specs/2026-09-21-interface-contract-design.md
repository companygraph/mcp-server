...
An **edge** is `{from, via, to, attrs}`, where `from` and `to` are entity references, `via` is the field or `Section.Column` that drew it, or `nested-in` for nesting, and `attrs` is an open record of the row's other columns, a resolved qualifier arriving as an entity reference. Today an entity's edges arrive flattened, the far end's `id`, `type` and `name` spread beside `via`; the edge names both ends so the same shape serves an entity's answer and a list of edges that belong to no single entity. Nesting is `nested-in` and not `owner`, because `owner` is also a field some schemas declare and a `via` names one thing.
...
`list_references` is the instance's edges, ownership included, filtered and paged. `entity` is an id and `direction` — `out`, `in` or `both`, default `both` — reads relative to it; without `entity`, direction has nothing to be relative to and is refused as `invalid_argument`. `via` matches the drawing field exactly, `Skills.Skill` or `nested-in`. `type` is the type of the far end when `entity` is given and of either end when it is not. With no argument at all it pages through every edge of the instance, which is allowed because it is bounded.
...
Each tool has a happy case and the empty cases it can produce: a search with no hit, a type that holds no entity, a reference filter that matches nothing, each still carrying its arrays and its `page`. Each error code has a case parsed against the error schema. The ambiguous one runs on the fixture the suite already builds in which two owners each hold one title, with the candidate ids asserted; the other fixture it builds, a name an identity and a profile share, is no longer an error anywhere, and shows `search` with `match: "name"` answering with both. Each paged tool is walked page by page at a small limit and the concatenation compared with the whole: nothing repeated, nothing missed, `hasMore` and `nextCursor` agreeing, and a cursor from another commit refused.
...
One release, the next minor after whatever `main` carries when this merges. `WORKING.md` makes a change a major when the taking repository must do more than re-pin, and here one does a little more: the tests that call the tools ship with the package and move with it, but mcp-blust-ch's own suite asserts the bare-name refusal of `fetch` and reads an evidence entry's flattened `id`, so its re-pin rewrites those two assertions, while mcp-companygraph-io re-pins and nothing else. Below 1.0 this package's minors have carried that much before, and a 1.0.0 is a statement about stability the owner makes on a day of its own, so the number stays a minor and the notes say what a deployment changes. For a client of a deployment it breaks, and the notes say so first, under `Interface`: `fetch` takes an id only and no longer returns `entity`; `get_entity`'s edges take the edge shape, are capped, and come with counts; `search` results are paged, `total` moves into `page`, and `matched` becomes objects; `list_entities` is paged and each entry gains `type`; a refused call carries structured content. Ids are formed as before.
