...
export const Entity = z.looseObject({
  ...
  path: z.string(), url: z.string().nullable(), image_url: z.string().optional(), fields: z.record(z.string(), z.unknown()), sections: z.array(Section),
...
// `to` is null for a reference whose type is read from its own row rather than declared by the
// schema (R9's `ref → by <Column>`, and, where that type is owned, `ref → by <Column> in` a
// second column naming the owner); `by` and `in` then name the columns that carry it, and stay
// null for every ordinary form.
const Relation = z.strictObject({ from: z.string(), via: z.string(), to: z.string().nullable(), form, by: z.string().nullable(), in: z.string().nullable(), array: z.boolean(), required: z.boolean(), min: count, max: count.nullable() });
...
// A JSON Schema, served as data by `describe_errors`. It is open because its keys are the
// vocabulary's and not this package's.
...
export const OUTPUTS = {
  ...
  describe_errors: answer({ errors: z.array(z.strictObject({ code: z.enum(CODES), when: z.string(), details: JsonSchema })), schema: JsonSchema }),
...
// A refusal. The SDK validates structured content on success and skips it on an error, and a
// tool declares one output schema, so this shape is registered with no tool: it is held by the
// suite and served as JSON Schema by `describe_errors`. Each code fixes its details.
