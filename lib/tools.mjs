...
import { z } from "zod";
import { ModelError, listTypes, describeSchema, describeRelations, listRules, describeRule, listEntities, getEntity, findEvidence, search, fetchEntity } from "./model.mjs";
...
export const TOOLS = [
  ...
  { name: "describe_schema",
    description: "One type's schema as the instance vendors it: file location, frontmatter fields, sections and their columns, purpose and writing rules. Its relations come beside it as data, both ways: the type that owns it, the types it owns, each reference it declares and each reference another schema declares to it.",
  ...
  { name: "describe_relations",
    description: "Every reference the schemas declare between types, as data read from their tables: the type that declares it, the field or Section.Column it is declared on, the type it names, its form (ref, ref? or qualifier, each explained in the answer), whether it is a list and whether it is required; and, apart from those, which type owns which. The whole vocabulary in one answer, for a diagram or an audit.",
    input: z.object({}),
