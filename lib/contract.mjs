...
export function checkAnswer(name, result) {
  // The tool is looked up first, before isError decides which schema to hold its answer to: an
  // unknown tool name has no schema to answer with either way.
  if (!OUTPUTS[name]) throw new Error(`${name} declares no output schema`);
