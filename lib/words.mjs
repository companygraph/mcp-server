...
// two words it is, and a possessive's apostrophe-s dropped before the split, so that "the owner's"
// is the word owner and not owner and a bare s. Each word is then reduced to its stem by Porter's
// algorithm, the one published in 1980, implemented here so that two deployments at one release
// give one answer and no dependency decides what a word becomes. English, because R14 makes every
// instance's names and prose American English; a visitor's German is the client's to translate
// before it asks.
...
export function words(text) {
  ...
  // The possessive goes here and not in the stemmer: the paper stems a bare s to nothing, and a
  // stem of nothing, held by every entity that writes a possessive and required like any other,
  // narrowed a visitor's question to the entities that happen to write one too.
  const owned = folded.replace(/['\u2019]s(?![\p{L}\p{N}])/gu, "");
  return owned.match(/[\p{L}\p{N}]+/gu) ?? [];
}
