...
export async function readGitHub({ repo, commit, sub, token = process.env.GITHUB_TOKEN, fetch = globalThis.fetch }) {
  ...
  // A picture beside a page is left out here as in readDir, and not fetched at all.
  const blobs = tree.filter((e) => e.type === "blob" && e.path.startsWith(sub) && !IMAGE_FILE.test(e.path));
  const files = new Map();
  let next = 0;
  const worker = async () => {
