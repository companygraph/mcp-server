...
// gives every declared reference with its form and how many of it a page may hold, the joins a
// schema declares between its tables, and the kind of list a section holds. Nothing here reads
...
const relationsOf = (s) => {
  if (!Array.isArray(s.schemaEdges) || !s.constraints)
    throw new ModelError("this snapshot was written before the schemas' declarations and constraints were kept in it; rebuild it with this release to ask what the types declare about each other");
  const ownership = s.schemaEdges.filter((e) => e.attrs?.type === undefined).map((e) => ({ owner: typeOf(e.to), owned: typeOf(e.from) }));
  const relations = [], joins = [], lists = [];
  ...
  return { relations, ownership, joins, lists };
};
...
export function describeSchema(s, type) {
  ...
  const { relations, ownership, joins, lists } = relationsOf(s);
  const own = {
...
export function listChecks(s) {
  if (!Array.isArray(s.checks))
    throw new ModelError("this snapshot was written before the checker's list of checks was kept in it; rebuild it with this release to ask what the instance is held to");
