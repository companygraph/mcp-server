import { z } from "zod";
import { ModelError, CODES, WHEN } from "./errors.mjs";
import { DETAILS, ErrorResult } from "./schemas.mjs";
import { paginate } from "./paging.mjs";
import { words, stems } from "./words.mjs";
...
// resolves to nothing is an error (R4); a name two owners of one type each hold is refused with
// every candidate named, never given to the first match; and a bare name is answered by
// `search` with match "name", with every entity that carries it, under whichever type.
...
// A file of the core the instance vendors, at the served commit: a schema, or the conventions
// the rules are read from. It is the instance's own copy, since the server answers from that and
// not from the meta-model's. Null where the repository, the commit or the core's place is not
// known, which a snapshot written before the place was kept also is.
const coreUrl = (s, file) => (s.repo && s.commit && typeof s.core.path === "string" ? `https://github.com/${s.repo}/blob/${s.commit}/${s.core.path}${file}` : null);
...
// than as an edge; it is served as one, from the owned to its owner, under `nested-in`, a name
// of this package's own, because `owner` is also a field some schemas declare and a `via` names
// one thing. The sort is stable, so two rows that draw the same edge keep the page's order.
const allEdges = (s) => {
  const byId = index(s);
  const nesting = s.entities.filter((e) => e.owner && byId.has(e.owner)).map((e) => ({ from: e.id, via: "nested-in", to: e.owner, attrs: {} }));
...
// The fields each type's schema declares `image` (core 0.38.0), read off the vocabulary the
// snapshot carries, so nothing here names profile. Empty for a core older than the type.
const imageFields = (s) => {
  const out = new Map();
  for (const schema of s.schemas ?? []) {
    const table = schema.sections?.find((x) => x.heading === "Frontmatter")?.tables?.[0];
    if (!table) continue;
    const field = table.columns.indexOf("Field"), type = table.columns.indexOf("Type");
    const names = table.rows.filter((r) => (r[type] ?? "").replace(/`/g, "").trim() === "image").map((r) => r[field].replace(/`/g, "").trim());
    if (names.length) out.set(schema.id.slice("core/".length), names);
  }
  return out;
};

// Where an entity's picture is served: the site the identity names, which is the site that
// publishes this model, at `images/<id>.<extension>`, the name the sites' build gives the copy.
// Null where the entity carries no picture, or the identity names no site to serve it from.
export const imageUrl = (s, e) => {
  const field = imageFields(s).get(e.type)?.find((f) => typeof e.fields?.[f] === "string" && e.fields[f]);
  const home = index(s).get(s.rootId)?.fields?.url;
  if (!field || typeof home !== "string") return null;
  return `${home.replace(/\/$/, "")}/images/${e.id}.${e.fields[field].split(".").pop()}`;
};
...
const serveEntity = (s, e) => {
  ...
  const image = imageUrl(s, e);
  return { ...rest, owner: e.owner ?? null, sections: serveSections(sections), url: fileUrl(s, e), ...(image ? { image_url: image } : {}),
...
const relationsOf = (s) => {
  ...
  for (const [type, c] of Object.entries(s.constraints)) {
    for (const { via, form, target, by, in: within, array, required, min, max } of c.references)
      relations.push({ from: type, via, to: target, form, by: by ?? null, in: within ?? null, array, required, min, max });
...
const READING = {
  ...
  "by and in": "A reference whose type is read from its row (R9) rather than declared by the schema: the row's `by` column names the type and, where that type is owned, its `in` column names the owner. `to` is null, because no one type is declared, and the relation is listed among what may reference every type.",
};
...
export function describeRelations(s, { type, direction, via } = {}) {
  ...
  // A relation with `to: null` reads its type from its own row (R9's `by`/`in` form), so it is
  // declared to none in particular and to every type at once: it stands on the declared-to side
  // of each of them.
  const readsAny = (x) => x.to === null && x.by !== null;
  return {
    relations: all.relations.filter((x) => named(x) && (type === undefined || (side !== "declared-to" && x.from === type) || (side !== "declares" && (x.to === type || readsAny(x))))),
...
export function describeSchema(s, type) {
  ...
  // See describeRelations: a relation with `to: null` reads its type from its own row and so
  // stands among what may reference every type, this one included.
  const readsAny = (x) => x.to === null && x.by !== null;
  const own = {
    ...
    referencedBy: relations.filter((x) => x.to === type || readsAny(x)).map(({ to, ...x }) => x),
  ...
  return { type, name: schema.name, tagline: schema.tagline, url: coreUrl(s, schema.path), sections: serveSections(schema.sections), relations: own, model: provenance(s) };
}
...
export function listRules(s) {
  ...
  return { tagline, url: coreUrl(s, "CONVENTIONS.md"), rules: rules.map(({ rule, title, part }) => ({ rule, title, part })), model: provenance(s) };
}
...
export function describeRule(s, rule) {
  ...
  return { ...found, url: coreUrl(s, "CONVENTIONS.md"), model: provenance(s) };
}
...
export function entityBy(s, { id, type, name } = {}) {
  ...
  // What is missing is what the caller left out: a name came without a type, a type came
  // without a name, or neither an id nor either of those came.
  const missing = name !== undefined ? "type" : type !== undefined ? "name" : "id";
  throw new ModelError("invalid_argument", "get_entity takes an id, or a type and a name", { details: { argument: missing, reason: "give an id, or a type and a name" } });
}
...
//
// The edges come a page at a time like every other list of them, because nothing bounds how many
// pages of an instance may name one skill. The order is the drawing page's type and then the
// order every list of edges has, so a page holds whole groups until its last, and a group cut
// by the end of a page goes on at the top of the next under the same key.
export function findEvidence(s, skill, { limit, cursor } = {}) {
  const byId = index(s);
  const held = byId.get(skill);
  const e = held?.type === "skill" ? held : resolveTyped(s, "skill", skill);
  const into = allEdges(s).filter((x) => x.to.id === e.id).sort((a, b) => cmp(a.from.type, b.from.type));
  const { items, page } = paginate(into, { limit, cursor }, s.commit);
  const evidence = {};
  ...
  return { skill: { ...ref(e), tagline: e.tagline }, evidence, page, model: provenance(s) };
}
...
// What a refusal looks like, for a client that is handed a schema for every answer and none for
// an error. Both parts are written from the one schema the suite holds every refusal to: each
// code with when it is raised and the schema of its details, for a reader choosing what to do
// about one, and the whole refusal as one JSON Schema, for a validator. Nothing here is the
// instance's; `model` stands beside it as it does beside every answer.
const jsonSchema = (schema) => {
  const { $schema, ...rest } = z.toJSONSchema(schema);
  return rest;
};
const ERRORS = CODES.map((code) => ({ code, when: WHEN[code], details: jsonSchema(DETAILS[code]) }));
const ERROR_SCHEMA = z.toJSONSchema(ErrorResult);

export function describeErrors(s) {
  return { errors: ERRORS, schema: ERROR_SCHEMA, model: provenance(s) };
}
