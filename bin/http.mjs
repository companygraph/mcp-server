...
try {
  const { values } = parseArgs({ options: { snapshot: { type: "string" }, port: { type: "string" }, "page-css": { type: "string" }, "page-icon": { type: "string" } } });
  if (!values.snapshot) { console.error("usage: companygraph-mcp-http --snapshot file [--port n] [--page-css file] [--page-icon file]"); process.exit(2); }
  ...
  createHttpServer(snapshot, { allowedHosts, pageCss, pageIcon }).listen(port, "0.0.0.0", () => {
    console.log(`companygraph-mcp-http on :${port}, commit ${snapshot.commit ?? "(none)"}, core ${snapshot.core.version}, hosts ${allowedHosts ? allowedHosts.join(" ") : "any"}, page css ${values["page-css"] ?? "built-in"}, icon ${values["page-icon"] ?? "none"}`);
  });
} catch (err) {
